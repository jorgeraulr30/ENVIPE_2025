...
setwd("C:/Users/jorge/OneDrive/Imágenes/Saved Pictures/Escritorio/UAQ/SEMESTRE 6/T1. ING MAT/BD ENVIPE CSV/2020 (bdf)")

# Cargar tablas
tper_Vic2 <- read.dbf("TPer_Vic2.dbf")
...
calcular_indicadores <- function(df1, df2, dominio) {
  ...
  corruptos <- suma_segura(df1 %>% filter(eval(dominio$filtro_vic1), AP5_5_11 == 1), fac_p)
  no_corruptos <- suma_segura(df1 %>% filter(eval(dominio$filtro_vic1), AP5_5_11 == 2), fac_p)
  ns_nc <- suma_segura(df1 %>% filter(eval(dominio$filtro_vic1), (AP5_5_11 == 9 | is.na(AP5_5_11))), fac_p)
...
write.csv(resultados_municipios, "resultados_municipios_2020.csv", row.names = FALSE)

print(resultados_municipios)

# Diagnóstico para El Marqués
el_marques <- tper_Vic1 %>% filter(CVE_ENT == "22", CVE_MUN == "011")
cat("Registros en El Marqués:", nrow(el_marques), "\n")
cat("Valores NA en FAC_ELE_AM:", sum(is.na(el_marques$FAC_ELE_AM)), "\n")
cat("Valores cero en FAC_ELE_AM:", sum(el_marques$FAC_ELE_AM == 0, na.rm = TRUE), "\n")
cat("Suma de FAC_ELE_AM:", sum(el_marques$FAC_ELE_AM, na.rm = TRUE), "\n")
cat("Suma de FAC_ELE (general):", sum(el_marques$FAC_ELE, na.rm = TRUE), "\n")